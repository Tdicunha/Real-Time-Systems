...
//imprime a tabela
    printf("\n");
    printf("\n| Thread |  Tempo Max de Computacao  |  Tempo Min de Computacao  |   Time Jitter |    Deadline   |  Tempo Maximo de Resposta |\n");
...
double subtrai_soma(struct timespec *A,struct timespec *B,int operacao){
	...
	if(operacao==1){								//operacao=1 Subtrai o tempos (s/ns)
		...
		A->tv_sec=A->tv_sec+B->tv_sec+(A->tv_nsec+B->tv_nsec)/NSECTSEC;	//operacao=0 Soma os tempos (s/ns)
...
void Thread1 (void *IN){
	...
	if ((subtrai_soma(start,&tempo_final,1)-(6-0.1-subtrai_soma(&espera,&termina,1))) > maxtempr1){//problema
...
void Thread3 (void *IN){
	...
	if ((subtrai_soma(start,&tempo_final,1)-(6-0.3-subtrai_soma(&espera,&termina,1))) > maxtempr3){//problema
